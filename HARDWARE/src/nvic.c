...
static u32 sysTickCnt=0;/*计数值*/

void nvicInit(void)
{
	NVIC_SetVectorTable(FIRMWARE_START_ADDR,0);/*设置nvic向量表的起始地址,第二个参数是相对FIRMWARE_START_ADDR偏移量*/
	NVIC_PriorityGroupConfig(NVIC_PriorityGroup_4);/*四位抢占优先级*/
}

extern void xPortSysTickHandler(void);/*freertos提供的中断函数*/
